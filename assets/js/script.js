$(document).ready(function(){
    ...
    $(document).on('change','#mobile_upload_input',function(event){
        ...
        var html = "<div class='tools'><div class='d-flex'><i class='append_square'></i><i class='append_circle'></i><i class='append_pointer'></i></div> <div class='save_and_remove'><a id='saveImage' ><i class='save_mark'></i></a><i class='delete_mark'></i></div></div>"
    ...
    $(document).on('change','#desk_upload_input',function(){
        ...
        reader.onload = function(event) {
            var imageData = event.target.result;
            localStorage.setItem('imageDataDesk', imageData);
        };
        reader.readAsDataURL(file);
        $('#desk_preview').addClass('active');
        $('.upload_label_main_2').hide();
        var html = "<div class='tools'><div class='d-flex'><i class='append_square_desk'></i><i class='append_circle_desk'></i><i class='append_pointer_desk'></i></div> <div class='save_and_remove_desk'><a id='saveImageDesk' ><i class='save_mark_desk'></i></a><i class='delete_mark_desk'></i></div></div>"
    ...
    $(document).on('click','#saveImage', function() {
        var issue_mark_html = $('#mobile_preview').clone(); 
        issue_mark_html.find('.tools').remove(); 
        localStorage.setItem('save_image',issue_mark_html.html());
        window.location.href = 'view.html';
    });
    
    var saveImageHtml = localStorage.getItem('save_image');
    $('#mobile_save_preview_append').html(saveImageHtml);
    $('#mobile_save_preview_append').find('img').attr('src',localStorage.getItem('imageData'));
...
$(document).on('click','.remove_checkpoints_desk',function(){
    ...
    $(document).on('click','#saveImageDesk', function() {
        var issue_mark_html = $('#desk_preview').clone(); 
        issue_mark_html.find('.tools').remove(); 
        localStorage.setItem('save_image_desk',issue_mark_html.html());
        window.location.href = 'view.html';
    });
    
    var saveImageHtml = localStorage.getItem('save_image_desk');
    $('#desk_save_preview_append').html(saveImageHtml);
    $('#desk_save_preview_append').find('img').attr('src',localStorage.getItem('imageDataDesk'));
    
    
    
    // var divToCapture = document.getElementById('mobile_upload_content');
